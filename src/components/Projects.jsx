    ...
    <section id="projects" className="portfolio-panel" ref={sectionRef}>
      <div className="panel-inner">
        ...
        <p className="panel-copy reveal" data-reveal>
          Representative projects where interface speed, process reliability, and maintainable code
          quality were all required at once.
        </p>

        <Carousel.Root count={PROJECTS.length} initialIndex={0} transitionMs={360} loop={false}>
          <Carousel.Panel>
            {({ activeIndex, count, goPrev, goNext, goTo }) => (
              <div className="projects-carousel reveal" data-reveal>
                ...
                <div className="projects-viewport">
                  <div className="projects-track" style={{ transform: `translateX(-${activeIndex * 100}%)` }}>
                    {PROJECTS.map((project, index) => (
                      <div key={project.title} className="projects-slide" aria-hidden={activeIndex !== index}>
                        <article
                          className="project-card focus-ring"
                          style={{ '--project-color': project.color }}
                          tabIndex={activeIndex === index && project.liveDemoUrl ? 0 : -1}
                          onKeyDown={(event) => handleProjectKeyDown(event, project.liveDemoUrl)}
                        ...
                        >
                          ...
                          {project.liveDemoUrl ? (
                            <a href={project.liveDemoUrl} target="_blank" rel="noreferrer" className="project-link focus-ring">
                              Open Live Demo
                            </a>
                          ) : null}
                        </article>
                ...
                <div className="projects-dots" role="tablist" aria-label="Project picker">
                  {PROJECTS.map((project, index) => (
                    <button
                      key={project.title}
                      type="button"
